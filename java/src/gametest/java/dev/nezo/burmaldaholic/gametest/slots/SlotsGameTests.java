...
/** Slot machines (v2) in a real world: stake, persisted tape, settlement, exits, restart, VIP gate, protocol abuse, jackpot API. */
public class SlotsGameTests {
	...
	/** SlotsApi's pool view follows the v2 Grand meter of the cabinet's machine; reset goes back to the seed. */
	@GameTest
	public void jackpotApiFollowsTheV2Pools(GameTestHelper helper) {
		MinecraftServer server = helper.getLevel().getServer();
		SlotsApi.resetJackpots(server, "gold");
		long seed = SlotsApi.jackpotPool(server, "gold");
		helper.assertTrue(seed == JackpotPoolsV2.get(server).meter(Machine.NETHER, SlotMachinesV2.def(Machine.NETHER), 4), "gold cabinet = Nether Inferno Grand");
		helper.assertTrue(SlotsApi.jackpotPool(server, "nope") == 0, "unknown tier");
		helper.succeed();
	}
