...
export default function generate() {
  const out = [];
  const gui = (name, img) => out.push(png(`${GUI}/${name}.png`, img));
  ...
  out.push(png(`${T}/font/core/card_index.png`, indexFontSheet()));
  out.push(json(`${JAVA_ASSETS}/font/core/card_index.json`, indexFontProvider()));
  out.push(png(`${T}/entity/core/cards/faces.png`, worldAtlas()));
  out.push(png(`${T}/entity/core/cards/chips.png`, hstrip(DENOMS.map((d) => chipDisc(d)))));
  for (let s = 0; s < 4; s++) for (let f = 0; f < 2; f++) out.push(png(`${T}/particle/core/card_suit_${s * 2 + f}.png`, suitParticle(s, f)));
