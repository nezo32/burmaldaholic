...
export default function generate() {
  validate(ALL_DEFS);
  const out = [];
  const sheetOut = (dir, name, img) => out.push(png(`${dir}/${name}.png`, img));
  const sprite = (dir, name, img, meta) => {
    out.push(png(`${dir}/${name}.png`, img));
    if (meta) out.push(mcmeta(`${dir}/${name}.png`, meta));
  };
  const rawMeta = (dir, name, value) => out.push(json(`${dir}/${name}.png.mcmeta`, value));
  ...
  for (const kind of ['heads', 'tails']) out.push(png(`${T}/item/extras/lucky_coin_${kind}.png`, luckyCoinItem(kind)));
  ...
  out.push(png(`${SPR}/wheel_hub.png`, vstrip(wheelHub())));
  ...
  out.push(png(`${T}/entity/extras/plinko_lamp.png`, plinkoLamp()));
  out.push(png(`${T}/entity/extras/plinko_ball.png`, worldBall()));
  out.push(png(`${T}/entity/extras/wheel_bulbs.png`, worldBulbs()));
  const fl = flakes();
  for (let i = 0; i < 4; i++) out.push(png(`${T}/particle/extras/foil_flake_${i}.png`, crop(fl, i * 4, 0, 4, 4)));
  return out;
}
