...
- **Cross-edition test vectors**: 20 fixed tapes per machine with the expected beat list (kind,
  start, duration), stored in `java/src/test/resources/fx/vectors/slots_timeline.json` and mirrored to
  `bedrock/test/fx/vectors/slots_timeline.json` (generated by the Java test, checked by the Bedrock test;
  location per `docs/architecture/animation.md` §3.4 — ⚠ CHANGED from `docs/design/animation/`). Both editions
