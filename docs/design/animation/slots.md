...
6. Bedrock (DDUI screen, classic fallback, titles, camera, in-world entity)
7. Slot Showdown presentation (both editions)
...
| SLOTS.md | engine | tape, `anticipationPlan`, tumble chain, feature steps (pure logic, both editions) |
...
### 1.2 Bedrock solo (`bedrock/src/games/slots/index.ts`, `render.ts`)

| Area | Finding | Verdict |
|---|---|---|
| Screen | classic `ActionFormData`: body = 3 glyph rows of the last grid, then buttons | Static. The form closes during the spin. |
| Reel motion | every 3 ticks the action bar shows a **new random grid** (`drawGrid(mathRng…)`), and a column freezes on the result at 50 / 75 / 100 % | Noise, not scrolling: symbols teleport. Filler comes from `Math.random`, not the strip. |
| Glyphs | `font/glyph_E2.png` 256 × 256 (16 px cells), 12 symbols U+E200–U+E20B | No win or blur variants. |
| Wins | action bar summary, `random.levelup` / `random.orb`; a title for jackpot and sevens | No tiering, no roll-up. |
| In-world | nothing (the blocks are static cubes) | Spectators see nothing. |
...
- **Bedrock** `slots/pvp/ui.ts` (branch `worktree-agent-a35f11d964d6bc392`): titles for HOT,
  Underdog, KABOOM and SWAP, and vanilla particles (`basic_flame_particle`, `large_explosion`,
  `portal_directional`) at the anchor or at the player. The reel animation is the same random-noise
  action bar as solo.
...
### 2.1 Fidelity contract (MUST, both editions; extends SLOTS.md §1.4 and global.md §1)

- **F1 — Tape first.** No frame starts before the client has the tape section it animates
  (Java: `spin` state with stops; Bedrock: after the engine drew the tape). The only exception is
...
- **Cross-edition test vectors**: 20 fixed tapes per machine with the expected beat list (kind,
  start, duration), stored in `docs/design/animation/slots-timeline-vectors.json` (generated by
  the Java test, checked by the Bedrock test). Both editions must produce identical lists.

### 2.3 Timing tokens (normal speed; turbo × 0.5; Bedrock rounds up to 50 ms)

| Token | Value | Source |
|---|---|---|
| `SPIN_UP` | 120 ms, back-kick −0.15 cell | SLOTS §10.2 |
| `V_MAX` | 25 cells/s (Java), 10 rows/s (Bedrock text) | SLOTS §10.2 |
| `STOP_R` | 600 + 150 × (r − 1) ms | SLOTS §10.2 |
| `LAND` | the last 350 ms before `STOP_R`, `outBack(1.2)` | SLOTS §10.2 |
| `ANTICIPATE_GAP` | 1 000 ms between later stops | SLOTS §10.3 |
| `SQUASH` | 80 ms down (scale-Y 0.90, X 1.06) + 120 ms back, per landing symbol | new |
| `WIN_DIM` | last stop + 150 ms, dim to 40 % over 150 ms | SLOTS §10.2 |
| `WIN_ALL` | 900 ms all-wins overview (new), then `WAY_CYCLE` 700 ms per symbol | SLOTS §10.2 + new |
| `TUMBLE` | 750 ms = explode 250 + fall 300 + pause 200 | SLOTS §10.2 |
| `WILD_EXPAND` | 300 ms + sticky frame 200 ms | SLOTS §10.2 |
| `FS_INTRO` / `FS_OUTRO` | 2 000 ms (skippable after 500) / roll-up + 1 500 ms hold | SLOTS §10.2 |
| `FS_SPIN_SCALE` | 0.8 × base timings | SLOTS §10.2 |
| `ROLLUP` | `clamp(600 + 900 × log10(1 + win/bet), 600, 8 000)` ms | SLOTS §10.1 |
| `DUR_SKIP` | 200 ms (reels) / 150 ms (other) | global/extras |
...
| Jackpot | static plaque + amount; no meter explosion; Bedrock no camera | no flash | scaled | Only mine: nearby cabinets' jackpot FX hidden |
| Epic GUI shake / Bedrock `camerashake` | none | — | — | — |
| Bedrock camera fade (Mega+) | none | none | — | — |
...
| Network | 1 `SpinSync` block update per spin (+1 per free spin or bonus step for spectators, coalesced to ≥ 10 t apart) | entity props: ≤ 7 `setProperty` per base spin, +1 per tumble/free spin/respin, all in the tick they happen |
| Script | — | one `runInterval(2)` per **active session** (not per spectator); ≤ 0.2 ms per tick per session; stopped when the form closes |
| DDUI | — | ≤ 1 Observable write per 2 ticks per label; jackpot label ≤ 1/s |
| Particles | ≤ 60 per burst, ≤ 150 for Grand, ≤ 3 `sendParticles` per event | ≤ 60 per burst via Molang `variable.count`, ≤ 4 `spawnParticle` calls per event |
| Sounds | ≤ 20/s per player (roll-up ticks ≤ 15/s) | same |
| Textures | 3 machine sheets ≤ 100 KB each; all GUI atlas sprites < 64 KB | glyph sheets 512 × 512 × 3; entity textures ≤ 256 × 256 (strip textures may be 16 × 720) |
...
Bedrock has no per-frame GUI drawing. The slot "screen" is told with three instruments that
complement each other: the **DDUI `CustomForm`** (SLOTS.md §10.6; live glyph reels in a form), the
**HUD** (titles, `updateSubtitle` roll-ups, action bar) for moments when no form is open, and the
**in-world cabinet entity** (`slot_reels`), which is the real showpiece and what spectators see.

### 6.1 Glyph planes (shared with SLOTS.md §2)

| Plane | Sheet | Content (same offsets in every plane) |
|---|---|---|
| U+E2xx | `packs/slots/RP/font/glyph_E2.png` | base symbols (rows 0–2 = Overworld, Nether, End), shared glyphs row 3 (SLOTS.md) + the additions below |
| U+E3xx | `glyph_E3.png` | **win** variant: the symbol brightened 20 % with a 1 px gold rim inside the cell |
| U+E4xx | `glyph_E4.png` | **blur** variant: vertical smear (as the Java `blur` frame) |

All three sheets: **512 × 512 (32 px cells)**, generated (§9.2). The sharper cells matter because
DDUI and titles render glyphs larger than chat. Row 3 additions (this file; append to SLOTS.md §2
"Shared glyphs"): U+E23B respin pip full, U+E23C respin pip empty, U+E23D anticipation arrow
(▼, gold), U+E23E ember blur (Hoard respin filler), U+E23F way node (small gold dot), U+E240 KABOOM
burst, U+E241 SWAP arrows, U+E242 TIME WARP clock, U+E243 wheel wedge divider, U+E244 ready check.

**[V] Tinting.** If `§` colour codes tint glyphs (research: likely, unverified), dim cells use
`§8` + base glyph and anticipation uses `§e`. Fallback when tinting does not work: no dimming
(win glyphs alone show the win), and the anticipation arrow U+E23D above the column.

### 6.2 DDUI machine form (MUST, after the DDUI spike B-S0)

Form layout (top to bottom; every item is an Observable bound component):

```
[title]   Nether Inferno                                   (machine name)
[label J] ⟨mini⟩ 5 040 · ⟨minor⟩ 15 210 · ⟨major⟩ 75 880 · ⟨grand⟩ 501 220      ≤ 1 update/s
[label H] (header line: tier word / feature banner / hint; empty at rest)
[label R] ⟨g⟩ ⟨g⟩ ⟨g⟩ ⟨g⟩ ⟨g⟩                                   3 rows × 5 glyphs, 2 spaces apart
          ⟨g⟩ ⟨g⟩ ⟨g⟩ ⟨g⟩ ⟨g⟩
          ⟨g⟩ ⟨g⟩ ⟨g⟩ ⟨g⟩ ⟨g⟩
[label S] Win 1 240 · Tumble ×3                                (status; roll-ups)
[button]  Spin (50)  ↔  Stop
[button]  Lower bet · [button] Raise bet · [button] Buy bonus (920) · Auto… · Paytable · Leave
```

**Frame loop.** One `system.runInterval(…, 2)` per open session builds the frame from the
`SlotTimeline` (§2.2) sampled at the current tick and writes only the Observables that changed.
It stops when the form closes (`isShowing()` false) or the timeline ends.

| Beat | Label R (reels) | Label H / S | Sound (`player.playSound`) |
|---|---|---|---|
| Spin press | the button's label switches to `slots.stop` | H cleared | `ui` click + `burmaldaholic.slots.spin_loop` |
| Spin-up (0–150 ms = 3 t) | columns move 1 row per frame with **base** glyphs | — | — |
| Full speed | each column advances 1 strip row per frame (10 rows/s) through the **real strip**, glyphs from the **blur** plane | — | — |
| Last 3 frames before a column's stop | base glyphs (reads as slowing) | — | — |
| Column r lands | the exact window for column r, drawn with **win-plane** glyphs for 1 frame (the "clunk" flash), then base (F4: no row overshoot) | — | `burmaldaholic.slots.reel_stop` pitch ladder (§4.2) |
| Scatter / bonus lands | that cell stays on the win plane for 2 frames | — | `slots.scatter_land` / `slots.bonus_land` with the pitch ladder |
| Anticipation | the anticipating column alternates `§e`-tinted blur and plain blur every frame [V]; the trigger cells blink win ↔ base at 2.5 Hz; fallback: U+E23D arrow above the column | H: empty (no text teases) | `slots.anticipation` looped, stopped with `stopsound` on land |
| Win overview (900 ms) | winning cells on the win plane, others `§8` [V] | S: roll-up every 2 t (`slots.win`) | `slots.win_small` at the end of the roll-up |
| Symbol cycle (14 t each) | only the current symbol's cells on the win plane | S: `slots.symbol_win` | — |
| Nice+ | as above | H: the tier word (`slots.tier.*`) alternates `§6` ↔ `§e` each 10 t (colour change, not a flash); upgrades Nice → Big → Mega → Epic as the status roll-up passes the thresholds | tier stems |

**Nether tumbles in the form**: the exploding cells turn into U+E23E ember glyphs for 2 frames,
then the column rows shift down one row per frame (the fall), new glyphs enter at the top, and S
shows `slots.tumble.mult`. Frame cost is identical to a spin.

**End sticky wilds**: an expanding Egg column fills from the Egg's row outward over 2 frames
(Egg glyph written to the neighbours) and stays on the win plane (glowing) for the feature; the
sticky reel is not advanced during later spins.

**Free spins**: H becomes the banner `slots.fs.title` · `slots.fs.name.<m>` for the intro (40 t,
skippable by Stop), then the counter `slots.fs.left` + (Nether) `slots.fs.multiplier` / (End)
`slots.sticky`; S shows `slots.fs.total` rolling after each spin. Music:
`burmaldaholic.slots.fs_music.<m>` (streamed loop, §8) started at the intro,
stopped at the outro with `stopsound`. Retrigger: H shows `slots.fs.retrigger` for 30 t. Outro: H =
`slots.fs.end` with the roll-up in the same label.

**Treasure Hunt**: label R becomes 3 rows of 5 chest glyphs (U+E234 closed, U+E235 open; opened
cells show the prize as text in S). Buttons: `slots.pick.open` (opens the next chest in reading
order, which is honest: SLOTS.md §1.2) and `slots.pick.open_all`. Opening a chest: 3 frames
(closed on the win plane → closed → open), then S = `slots.pick.prize`, sound
`slots.chest_open`. Creeper: the cell shows U+E236 alternating `§f` / `§7` every 10 t for 30 t
(swell), then H = `slots.pick.creeper`, sound `slots.creeper_hiss`, `dimension.spawnParticle`
smoke puff at the cabinet (no explosion, no damage). End: remaining chests open with their prizes
dimmed `§8` 2 per frame.

**Piglin's Hoard**: R = 3 rows × 5 cells, each cell `⟨coin⟩` + short value (`5×`; jackpot coins show the badge
glyph U+E230–U+E232 instead of a value) or the empty glyph U+E237;
H = `slots.bonus.hold` + pips U+E23B/U+E23C (●●○). Respin: empty cells show U+E23E (ember blur,
D5) for 8 frames, landing 1 cell per frame in reading order. New coin: that cell on the win plane
for 3 frames, pips refill, sound `slots.coin_land` + `slots.respin_reset`. Collect: each coin goes
to the win plane in turn (1 per 2 t) while S rolls up.

**Dragon Wheel**: R becomes one line: a 7-wedge strip `│10×│UP│12×│15×│MINI│10×│20×│` with the
pointer glyph U+E238 above the middle wedge; the strip scrolls 1 wedge per frame at speed,
slowing with `outCubic` (frame gaps grow from 2 t to 8 t), and lands on the tape's wedge; a tick
sound per wedge (`slots.wheel_tick`, pitch rising). UP: H = `slots.wheel.up`, then the next ring
label `slots.wheel.ring.*`. The cabinet's in-world wheel spins in sync (§6.6).

**Jackpots in the form**: Mini/Minor stay in the form: H = `slots.jackpot.won` (tier word in the
badge colour), J flashes that meter's text to `§e` for 20 t; world particles + `jackpot` sound.
**Major/Grand** get a cinematic (MUST, reduce motion → titles only): the server closes the form
(`uiManager.closeAllForms`), then:

| t (ticks) | Action |
|---|---|
| 0 | `camera.fade` gold `#FFD640` fadeIn 0.1 s / hold 0.1 / out 0.4 (flashes on) |
| 2 | `setCamera('minecraft:free', { location: cabinet front 2.2 blocks out + 1.4 up, facingLocation: reel face, easeOptions: { easeTime: 1.0, easeType: InOutSine } })` |
| 4 | title `slots.jackpot.won` (fade 5/60/10) + subtitle roll-up via `updateSubtitle` every 2 t for 40 t (Major) / 60 t (Grand) |
| 6 | `jackpot_burst` + `coin_burst` (60) at the cabinet; Grand: `camerashake add @s 0.2 0.6 positional` |
| 50 / 70 | `camera.clear()` |
| 60 / 80 | the form re-opens on the machine (status = spin total) |

### 6.3 Classic fallback (`slots.bedrock.ddui` = false or DDUI failed)

- Machine `ActionFormData` between spins (SLOTS.md §10.6); body: the jackpot line, the last window
  (3 glyph rows, winning cells on the win plane), the result line.
- During the spin the form is closed and the **action bar** carries the reels: 3 glyph rows,
  updated every 2 ticks, with the same frame rules as §6.2 (blur plane, land flash, anticipation
  arrow). The action bar keeps priority `HudPriority.game` for the duration.
- Win: the action bar shows the last window + `slots.win` rolling (every 2 t); Big+ via
  `fx.celebrate` (title + `updateSubtitle`, global.md §2.6) with the slot tier word (D4).
- Features: titles for the banners (`slots.fs.title`, `slots.bonus.*`), the action bar for
  counters, and an `ActionFormData` for "Open a chest" / "Open all" in the hunt.
- The in-world cabinet (§6.6) always animates too, so the player can simply watch the machine.

### 6.4 Skip, turbo, settings (Bedrock)

- Skip: the **Stop** button (DDUI), or sneaking while within 4 blocks of the cabinet (both paths,
  global `fx.skip_bedrock` hint shown once per session in H).
- Turbo: per-player dynamic property `burmaldaholic:anim.turbo` (Casino Menu → Settings); frame
  gaps stay 2 t but beats are halved.
- Reduce motion: no reel scrolling (columns switch blur → landed at their stop ticks), no camera,
  no `camerashake`, no fades, roll-ups in 2 steps; particle `variable.count` × 0.3.

### 6.5 Bedrock "watch the cabinet" mode (NICE)

A per-player toggle `slots.fx.cabinet_view` (Casino Menu → Settings): pressing Spin closes the form
and eases the camera to the cabinet front (`minecraft:free`, 0.6 s `InOutSine`) for the whole
round; amounts go to the action bar and titles; the form re-opens at the end (or on sneak). Off by
default (forced camera moves are the most motion-sensitive effect). Disabled with reduce motion.

### 6.6 In-world cabinet: `burmaldaholic:slot_reels` entity

One AI-free entity per cabinet (research §3.2, §7), spawned by the slots module when the block is
placed or its chunk loads (worldgen croupier pattern), removed with the block. Collision box 0.01,
no gravity, `damage_sensor` none, not pushable.

#### 6.6.1 Files (per machine m ∈ overworld, nether, end)

| File | Content |
|---|---|
| BP `entities/slots/slot_reels.json` | identifier, properties (below), components |
| RP `entity/slots/slot_reels.entity.json` | textures (strip, blur, overlays, cabinet), geometry per machine via `q.property('burmaldaholic:machine')` render-controller arrays, 7 render controllers (5 reels + overlay + cabinet), animations, controllers, `scripts.pre_animation` computing `v.reel_off_r` |
| RP `models/entity/slots/slot_reels_<m>.geo.json` | bones below (generated) |
| RP `animations/slots/slot_reels.animation.json` | `spin_r`, `land_r` (r = 0…4), `win_blink`, `tumble_pop`, `sticky_expand_r` (r = 1…3), `hold_lock`, `wheel_spin_<ring>`, `wheel_zoom`, `idle_marquee` (generated Molang constants) |
| RP `animation_controllers/slots/slot_reels.ac.json` | `controller.animation.slot_reels.reel<r>` (idle → spin → landed), `.fx` (idle/win/big/jackpot/feature) |
| RP `render_controllers/slots/slot_reels.rc.json` | `controller.render.slot_reels.reel<r>` (part_visibility = only `reel<r>`; texture = blur while `v.spin_r`, else the strip; `uv_anim` offset [V]), `controller.render.slot_reels.overlay` (win frames, sticky, hold, plates), `controller.render.slot_reels.cabinet` |
| RP `textures/entity/slots/<m>_strip_<r>.png` | 16 × (16·L_r) px: the reel strip r of SLOTS.md Appendix A, one symbol per 16 px cell |
| RP `textures/entity/slots/<m>_blur.png` | 16 × 64 blur strip (loops) |
| RP `textures/entity/slots/<m>_overlay.png` | 64 × 64: win frame, sticky egg column, coin, empty cell, pips, ladder plates (digits allowed), wheel |

#### 6.6.2 Properties (all `client_sync`)

| Property | Type / range | Set when | Used for |
|---|---|---|---|
| `burmaldaholic:machine` | int 0–2 | spawn | geometry/texture arrays |
| `r0`…`r4` | int 0–44 | spin start (all 5, same tick) | landed strip offset `r / L` |
| `state` | enum idle, spin, land, win, big, feature, jackpot | beats | fx controller, marquee |
| `seq` | int 0–255 | spin start (+1 mod 256) | restarts the reel controllers each spin |
| `win` | int 0–32 767 | win show | 15-bit mask of winning cells (overlay frames) |
| `sticky` | int 0–7 | End FS stick | sticky column overlays |
| `hold` | int 0–32 767 | Hoard lock / new coin | coin overlays |
| `mult` | int 0–10 | tumble step / FS ladder | plate index |
| `wheel` | int 0–399 | wheel spin start (ring × 100 + segment) | wheel target angle |
| NICE (D3): `g0`, `g1`, `g2` | int 0–1 048 575 | tumble chain end | final window rows packed 4 bits × 5 cells (< 2²⁴, exact in Molang floats); overlay cell bones show it |

13 properties (16 with NICE), within the ~32 limit **[V]**.

#### 6.6.3 Geometry and animation

- Bones: `root` → `cabinet` (static upper body, marquee quads), `reel0…reel4` (one 3 × 9 px quad
  each, behind a `glass` quad), `overlay` → `cell_00…cell_24` (15 frame quads), `sticky1…3`
  (column quads), `coin_00…coin_24`, `pips`, `plate`, (End) `wheel` → `ring_outer`,
  `ring_middle`, `ring_core`, `pointer`. NICE: `final_rc_s` bones (15 cells × 11 symbols =
  165 quads) with `part_visibility` generated from `g0…g2`.
- **Spin**: `spin_r` sets `v.spin_r = 1` and the render controller shows the looping blur texture
  with `uv_anim` offset `math.mod(q.anim_time * 2.5, 1)` (10 cells/s over a 4-cell loop).
- **Land**: the server calls `entity.playAnimation('animation.burmaldaholic.slot_reels.land_r',
  { controller: 'controller.animation.slot_reels.reel<r>', nextState: 'landed' })` at each stop tick
  of the `SlotTimeline` (same ticks as the player's form). `land_r` sets `v.spin_r = 0` at 0.0 and
  moves bone `reel<r>` y: +3 px → −0.5 px (0.25 s) → 0 (0.35 s), `catmullrom` (an `outBack` look).
  The strip texture shows offset `q.property('burmaldaholic:r<r>') / L_r` **[V: per-controller
  `uv_anim`]**. Fallback if per-controller `uv_anim` is not available: the render controller picks
  from a **texture array of pre-cut windows** `array.win_r[q.property('burmaldaholic:r<r>')]`
  (45 textures of 16 × 48 per reel, generated), which is exact and needs no UV maths.
- **Win**: `win_blink` toggles the `cell_*` frames selected by the `win` mask at 2 Hz for 3 s
  (visibility via generated per-bit expressions `math.mod(math.floor(q.property('burmaldaholic:win')
  / 8), 2) == 1`).
- **Tumbles (MUST, D3)**: per step, `tumble_pop` pulses the winning frames orange and the server
  spawns `ember_burst` at the reel face; `mult` updates the ladder plate. The reels keep the first
  landed window. **NICE**: at chain end, `g0…g2` are set and the final-window overlay replaces the
  reel view until the next spin.
- **Sticky**: `sticky_expand_r` scales the `sticky<r>` column from 1/3 to 1 (0.3 s) when its bit
  turns on; an emissive shimmer loop follows.
- **Hoard**: coin bones visible by `hold` bits; `hold_lock` pops a new coin (scale 1.3 → 1, 0.2 s).
- **Wheel (End)**: `wheel_spin_<ring>` rotates `ring_<ring>` with
  `v.a = v.from + (v.to − v.from) * (1 − math.pow(1 − math.min(q.anim_time / T, 1), 3))`, where
  `v.to` comes from a generated table indexed by `q.property('burmaldaholic:wheel')` plus 3 turns;
  the pointer flapper uses a generated sawtooth of the wedge crossings (extras-pvp §10.2 pattern).
- **Marquee / state**: the fx controller swaps the marquee UV frames (idle chase, spin fast, win
  blink, big alternate, jackpot rainbow, FS theme colour).
- **LOD [V]**: controllers skip bone animation when `q.distance_from_camera > 24`.

#### 6.6.4 Block flipbooks

`packs/slots/RP/textures/flipbook_textures.json`: the cabinet base front (lower block) gets a
4-frame bulb chase (`ticks_per_frame` 4 / 3 / 2, End with `blend_frames`), same PNG strips as
Java's `.mcmeta` (global.md §4.13). The reel window itself is on the entity, never on the block
texture (no fake spinning).

#### 6.6.5 World FX (Bedrock)

Same table as Java §5.3 with `dimension.spawnParticle` + `MolangVariableMap` (`variable.count`)
and `dimension.playSound` for Big+ (neighbours hear it), `player.playSound` otherwise. Particles
(§9.3): `burmaldaholic:coin_burst`, `sparkle`, `confetti`, `jackpot_burst`, `ember_burst`,
`void_motes`.
...
- Per SLOTS.md §9.5: the action bar shows your **middle row** as 5 glyphs + points, updated every
  2 ticks during ROUND_SPIN (real strip, blur plane, land flash), then
  `gui.burmaldaholic.pvp.match.bar` for 60 t.
- Titles: round start `Spin r/N` + subtitle HOT (glyph + name); hazards: `kaboom_title` /
  `swap_title` / `time_warp_title` to the affected players (fade 0/20/8) — unchanged from the
  branch, plus sounds as Java.
- **In-world (MUST)**: every participant's cabinet `slot_reels` entity spins that participant's
  tape (the same `r0…r4`, `land_r` calls), so the whole group sees a row of cabinets landing
  together. HOT: `basic_flame_particle` above cabinets whose win includes the HOT symbol; hazards:
  KABOOM `large_explosion` puff + `camerashake add @s 0.15 0.3` for the victim only (not with reduce
  motion), SWAP `portal_directional` at both cabinets, TIME WARP `sparkle` ring.
- Features: montage in the action bar (the badge text + points counting, ≤ 60 t).
- Final Reveal: extras-pvp §9.4 Bedrock.
...
| Id | MUST composition (Java event / Bedrock sound) | Played by code as |
|---|---|---|
| `slots.spin_loop` | `entity.minecart.riding` p 1.6 v 0.25 / `minecart.inside` | Java: looping `AbstractTickableSoundInstance`, fade in 150 ms / out 120 ms; Bedrock: re-played every 20 t, stopped with `stopsound` |
...
| `slots.anticipation` | `block.beacon.ambient` (Java pitch ramp 0.8 → 1.3; Bedrock 3 re-plays at 1.0 / 1.15 / 1.3) | loop |
...
| Asset | Path (`bedrock/packs/slots/…`) | Size / notes | Count |
|---|---|---|---|
| Glyph planes | `RP/font/glyph_E2.png`, `glyph_E3.png`, `glyph_E4.png` | 512 × 512, 32 px cells | 3 |
| Entity strips | `RP/textures/entity/slots/<m>_strip_<r>.png` | 16 × 16·L_r | 15 |
| Entity blur / cabinet / overlay | `RP/textures/entity/slots/<m>_{blur,cabinet,overlay}.png` | 16 × 64 / 64 × 64 / 64 × 64 | 9 |
| Window cuts (fallback [V]) | `RP/textures/entity/slots/win/<m>_<r>_<t>.png` | 16 × 48; generated only if B-S0 shows per-controller `uv_anim` fails | 585 |
| Geometry | `RP/models/entity/slots/slot_reels_<m>.geo.json` | generated (≤ 300 cubes/quads; 465 with NICE g-overlay) | 3 |
| Entity / animations / controllers / render controllers | `BP/entities/slots/slot_reels.json`, `RP/entity/…`, `RP/animations/…`, `RP/animation_controllers/…`, `RP/render_controllers/…` | generated Molang constants | 5 |
| Particles | `RP/particles/{coin_burst,sparkle,confetti,jackpot_burst,ember_burst,void_motes}.json` + atlas `RP/textures/particle/burmaldaholic_slots.png` 64 × 64 | shared names with Java | 6 + 1 |
| Flipbooks | `RP/textures/flipbook_textures.json` + 3 strips | 16 × 64 | 4 |
| Button icons (DDUI / classic) | `RP/textures/ui/burmaldaholic/slots/{spin,stop,bet_down,bet_up,buy,auto,paytable,leave,chest}.png` | 32 × 32 | 9 |
| Sounds | `RP/sounds/sound_definitions.json` | §8 (vanilla paths) | 26 ids |

Bedrock total: **≈ 45 files** (+ 585 fallback cuts only if needed).

### 9.3 Particle definitions (both editions, same ids)
...
| SX2 | `SlotTimeline` pure builder (beats of §2.2/§2.3, turbo, reduce-motion variant) + unit tests + cross-edition vectors JSON | Java `games/slots/logic/anim/SlotTimeline.java`; Bedrock `games/slots/logic/timeline.ts`; `docs/design/animation/slots-timeline-vectors.json` | SLOTS engine (tape, `anticipationPlan`, tumble chain) | 1.5 d |
| SX3 | Sound definitions for §8 (both editions) | `java/src/main/sounds/slots/sounds.json`, `bedrock/packs/slots/RP/sounds/sound_definitions.json` | global J2/B2 | 0.5 d |
...
| Id | Task | Files | Depends on | Est. |
|---|---|---|---|---|
| BS0 | **Spike**: DDUI `CustomForm` + Observable label at 2 t with 3 glyph rows (flicker, coalescing), glyph `§` tinting, title with glyphs, per-render-controller `uv_anim`, `q.distance_from_camera`; write findings into this file's [V] items | scratch pack | — | 1 d |
| BS1 | Frame engine: sample `SlotTimeline` → glyph rows (strip scroll, blur/win planes, land flash, anticipation tint/arrow, dim) | `games/slots/present/frames.ts` (+ tests) | SX2 | 1.5 d |
| BS2 | DDUI machine form: layout, Observables, button bindings (Spin ↔ Stop), loop lifecycle, rate limits | `games/slots/present/ddui-form.ts` | BS0, BS1 | 1.5 d |
| BS3 | Classic fallback: action bar reels, forms, titles, hunt ActionForm | `games/slots/present/classic.ts` | BS1 | 1 d |
| BS4 | Features in the form: tumbles, sticky, FS banners/counters/music, hunt, hoard, wheel strip | `games/slots/present/features.ts` | BS2 or BS3 | 2 d |
| BS5 | Big wins and jackpots: `fx.celebrate` with slot words, Major/Grand cinematic (close form, fade, camera, titles, reopen) | `games/slots/present/celebrate.ts` | B1, BS2 | 1 d |
| BS6 | `slot_reels` entity: BP/RP files from SX1 generator, spawn/link/remove lifecycle, chunk-load relink | `packs/slots/BP/entities/…`, `packs/slots/RP/…`, `games/slots/cabinet.ts` | SX1 | 2 d |
| BS7 | Entity driver: props, `land_r` calls at timeline ticks, win mask, tumble pops, sticky, hold, wheel, fx state; NICE `g0…g2` | `games/slots/present/cabinet-driver.ts` | BS6, SX2 | 1.5 d |
| BS8 | Particles, flipbooks, sound definitions wiring | `packs/slots/RP/particles/*`, `flipbook_textures.json` | SX1, SX3, B3 | 0.5 d |
| BS9 | Showdown: action-bar middle row, titles, participants' cabinets spin their tapes, hazard FX | branch `slots/pvp/ui.ts` | BS1, BS7 | 1 d |
| BS10 | Settings (turbo, reduce motion, NICE cabinet view), sneak-to-skip | `games/slots/present/settings.ts` | B1 | 0.5 d |
...
- Both: fidelity contract F1–F10, `SlotTimeline` with cross-edition vectors, honest anticipation,
...
- Java: sheet-based reels with spin-up, blur, `outBack` landing, squash, drum shading; win show with
  ...
  Showdown screen §7.1.
- Bedrock: DDUI form with live glyph reels on the real strip (after BS0; classic fallback always
  kept), features in the form, Major/Grand camera cinematic (opt-out via reduce motion),
  `slot_reels` entity with stops, win mask, sticky, Hoard, wheel and tumble plate, flipbooks,
  particles; Showdown §7.2 including in-world cabinets.
...
- Java: lever, glass sheen sweep, Grand rainbow rim, parallax extra layer, shader shimmer on banners
  (global phase 3).
- Bedrock: "watch the cabinet" mode (§6.5), final-window overlay for Nether tumbles (`g0…g2`, D3),
  LOD via `q.distance_from_camera`.
- Idle flourishes on the BER / entity (Java screen idle flourishes are MUST: they are cheap).

### 12.3 Open questions

1. **Tier thresholds** (Q1): slots use 5/15/40/100 × bet (SLOTS.md §10.1), global uses 10/25/50.
   Keep both (slot screen vs broadcast) or align? This spec keeps both.
...
3. **Glyph tinting** by `§` codes (§6.1) and **per-controller `uv_anim`** (§6.6.3) — both have
   fallbacks.
4. **Music ids**: vanilla music event names differ between editions; verify, or ship the NICE loops.
5. **Deviations D1–D6** (§0.3) need the slots designer's OK; D4 needs a small `CelebrationOverlay` /
   `fx.celebrate` API change from the global owners (caller-supplied tier word).
