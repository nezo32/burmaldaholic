...
> `docs/design/visual/extras.md`, and its art is generated by `tools/assets/modules/extras.mjs` (Java outputs only).
...
### 0.3 Outcome fidelity rules (MUST; they extend `global.md` §6)
...
| Item | Java |
|------|------|
| Screen draw cost | ≤ 1.5 ms per frame at GUI scale 3, ≤ 600 blits, no per-frame allocation (reuse arrays; the scratch mask is a `long[]` per cell) |
| GUI particles | the shared pool (≤ 96 live); foil flakes use it too |
...
| Fx entities | — |
| Title / action-bar frames | n/a |
| Sounds | ≤ 20/s per player; the wheel flapper plays every 2nd–3rd peg at speed |
| World particles | ≤ 3 `sendParticles` calls per event, ≤ 60 particles per burst |
...
- Every texture of this file comes from an **extras/PvP module of S1**: `scripts/fx_assets/extras_pvp.py`, imported by
  ...
  `…/textures/gui/sprites/burmaldaholic/{extras,pvp}/…`, `…/textures/gui/extras/…` (code-indexed
  sheets), `…/textures/entity/extras/…`, `…/textures/particle/burmaldaholic/…` and the shared glyph sheet cells (§10.5).
...
### 3.2 Visual design — the wheel face
...
| Part | Java (branch `a0d8581…`) | Problems |
|------|--------------------------|----------|
| Invites | chat line with [Accept]/[Decline], goat horn | Java has no toast, so invites are easy to miss in combat and chat spam. No countdown is visible. |
| Lobby | `PvpLobbyScreen` rows; the timer as text | Rows pop in and out. There is no join fanfare, the empty seats are just text and the timer is plain text. |
| Countdown | a digit on the coin (duel only) | There is no shared countdown component on Java; each mode improvises. |
| Final Reveal | titles via `PvpScreensPresenter` + bells + angry villagers + totem; `PvpResultScreen` reveals rows every 10 t | **Java plays no drumroll** (no `basedrum` anywhere on the Java branches), although PVP.md §3.11.4 requires it. The Java result rows are not synced with the server's cue ticks (`STEP = 10` t vs the 20 t cues). There is no visual build-up. |
| Grudge / revenge | `PvpModule`: title + ravager roar | Title only. |
| Win streaks | chat messages | No visual at all. |
| Taunts | chat + villager yes/no | Taunts are not tied to the speaker in the world. |
...
| Tier | Java |
|------|------|
| heating (3) | `CasinoToast` with 1 flame (U+E170 glyph, drawn 2×), the translated message, and `block.fire.ambient` |
| rampage (5) | Toast with 2 flames, a red border, and an animated flame sprite (8 frames); `entity.blaze.shoot` at 0.5 |
| legendary (10) | Toast with a gold border + an 8-frame gold flame + `item.totem.use` in the radius (exists); the server-wide chat line (exists) |
| streak broken | Toast with a snuffed-flame sprite (3 frames: flame → smoke), `block.fire.extinguish` |
...
### 10.3 Sound events (ids shared by vanilla composites in the MUST phase, as in `global.md` §2.7)
...
| Id (`burmaldaholic:x`) | Status | Java composition | Description for the sound designer | Subtitle key |
|---------------------------------------------------------|--------|--------------------------------------------------|------------------------------------|--------------|
...
Java: add the new ids to `java/src/main/sounds/extras/sounds.json` with subtitles (§13). Pitch variation is set in code.
...
|------|-----------------|------|
| Coin Flip | Java sprite coin on the shared sheet, half-turn frame function, bounce, glint, balance hold, `ItemDisplay` toss | Bedrock hand attachable |
| Coin Flip Duel | VS intro, countdown overlay, open-ended toss → true face, coin slides to the winner, pot chips, chain tracker, heat 1–5, ALL SQUARE clink | Java midpoint display coin (MUST if V2 passes), DDUI duel form |
| Wheel of Fortune | wedge icons, baked face, pegs + flapper + ticks, pull-back and settle, stop pop-out, creeper swell, skip, Java BER with synced spin | attract breathing, 3D PiP wheel |
| Wheel Party | slice grow-in, heads on arcs, owner label, true "By a hair" crawl, No-more-bets slam, extrusion + crown + chips, BER party mode | Bedrock `party_face` spin, Java winner `TextDisplay` |
| Plinko | gravity + hop, peg flash, column pitch, trail, bin press + label pop, BER ball + lamps | multi-ball queue, DDUI board |
| Plinko Battle | mini boards with live balls, ranking slide, gold underdog ball, EDGE eruption, foil-capped final bins revealed per place | peg flash on mini boards, floating scoreboard |
| Scratch Cards | ticket art, symbols, drag scratching with the mask, flakes, auto-swipe, honest end emphasis, top-prize holo, creeper char | DDUI live scratching, held-card attachable |
| Scratch Showdown | swipe ripple, flakes, creeper swell → spark → burn, foot stamp, trio frames, score roll up/down, final foil per place | rank medals, DDUI table |
| Shared PvP | challenge toast, lobby row motion, countdown overlay, **Java drumroll**, plaque-flip Final Reveal on server cues, grudge clash, streak toasts, taunt bubbles (if V2/V3 pass) | Bedrock pictogram taunt bubbles, animated floating scoreboard |
...
| SX2 | Pure animation logic with **identical test vectors** (§14) | Java `games/extras/logic/anim/{CoinAnim,WheelAnim,PlinkoAnim,ScratchWipe}.java` + tests | — |
| SX3 | Strings (§15) into STRINGS.md and the lang files | `docs/design/STRINGS.md`, `java/src/main/lang/{extras,pvp}/*` | — |
| SX4 | Sound ids (§10.3): Java `sounds.json` entries + Bedrock `sound_definitions.json` for the extras pack | `java/src/main/sounds/extras/sounds.json` | J2, B2 |
...
## 14. Fidelity tests (identical vectors; they gate the review of every task)
