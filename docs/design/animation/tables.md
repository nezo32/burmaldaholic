...
Dice Duel against the house and PvP (§11.5, UI.md §9, PVP.md §10.6),. Rules, odds and
...
**Update (visual redesign, 2026-09-24):** the look and the art are specified in
[`docs/design/visual/tables.md`](../visual/tables.md) and generated by `tools/assets/modules/tables.mjs`.
The project is now **Java only**: every Bedrock storyboard, entity, glyph and file in this spec is dropped.
The choreography changes listed in visual §9 are applied below: the wheel is a pre-rendered frame set and is
never scaled, the layout dims in place, the dolly stands on the stack, the craps back wall is part of the
layout, and the duel is a full-screen arena. §4 is superseded by visual §8.
...
- §5 Developer task breakdown, in parallel lanes
...
- The path math lives in pure classes with golden test vectors:
  - Java: `…/logic/*Path.java`, with tests in `src/test`.
  - The test vectors are a JSON file: `java/src/test/resources/fx/vectors/tables.json`, generated once by
    the Java test (`FX_DUMP_VECTORS=1`) and then frozen.
...
### 0.2 Easing vocabulary (same names)
...
### 0.8 Sounds (new ids; the same ids)
...
| Id | Module | Description (the feel to aim for) | Java vanilla map (pitch / vol) |
|----|--------|--------------------------------|---------------------------------|
| `roulette_ball_roll` | roulette | Short rolling tick; repeated every 250→600 ms with falling pitch while the ball orbits | `block.stone_button.click_off` 1.9/0.25 |
| `roulette_ball_drop` | roulette | Hard clack as the ball hits a deflector diamond | `block.bamboo_wood.hit` 1.6/0.7 |
| `roulette_ball_bounce` | roulette | Lighter clatter as the ball hops across the frets | `block.bamboo_wood_button.click_on` 1.4/0.6 |
| `roulette_ball_settle` | roulette | Soft click as the ball sits in its pocket | `block.wooden_button.click_off` 1.1/0.6 |
| `roulette_bell` | roulette | Dealer's "no more bets" bell (one strike) | `block.bell.use` 1.6/0.4 |
| `roulette_dolly` | roulette | Glassy tink as the win marker lands | `block.amethyst_block.hit` 1.3/0.8 |
| `chip_sweep` | core | Chips raked across felt | `item.bundle.remove_one` 0.8/0.7 |
| `dice_throw` | extras | Whoosh plus rattle as the dice leave the hand | `entity.snowball.throw` 1.3/0.5 |
| `dice_bounce` | extras | Dice bouncing on felt (per floor contact) | `block.wool.hit` 1.2/0.9 |
| `dice_wall` | craps | Dice hitting the rubber back wall | `block.wood.hit` 1.5/0.8 |
| `dice_cup` | extras | Dice rattling in the cup (loop of 3 hits) | `block.bamboo_wood.step` 1.8/0.6 |
| `craps_puck` | craps | Heavy plastic puck set down | `block.stone_button.click_on` 0.6/0.8 |
...
### 1.3 Ball path, shared (`RouletteBallPath`)
...
| 600–1000 | The layout **stays in place, dimmed** (no fractional scaling of pixel art). The big wheel (`wheel_shadow` + `wheel_bowl_<theme>` + a `wheel_head` frame, visual §3.1) fades in and rises 8 px into place, centred on the felt (`inOutCubic`, 400 ms). |

**SPIN** (S = 5000 ms, §1.3). Wheel layers, bottom to top:
1. The bowl (static wood and track).
2. 8 deflector diamonds (static).
3. The head, which rotates: one of **74 pre-rendered frames** (half-pocket steps; pockets, frets, **upright
   baked pocket digits**), or a blur frame above 180°/s (visual §3.1).
...
- Dimmed layout (behind the wheel): nothing moves on it during the spin, apart from a slow 1 Hz shimmer on your
...
| t | Beat |
|---|------|
| 0–400 | The big wheel fades out and sinks 8 px; the mini wheel (left of the layout) switches to the result's frame (`wheel_mini_<theme>`, ball on the result). The layout undims (`inOutCubic`). |
| 400–650 | The **dolly** drops onto the winning number cell (on top of the chips there, visual §3.4) from 24 px above, with `outBounce`. Its shadow grows from 30 % to 100 %, and `roulette_dolly` plays. The winning cell's outline turns gold, and every covering outside box (dozen, column, red and so on) gets a gold outline with a 60 ms stagger in reading order. |
...
- **NICE:**
  ...
  - ~~Fallback F1: 8 pre-rendered rotation frames~~ superseded: the head is always a pre-rendered frame set (visual §3.1).
...
Layout changes (MUST):
- A **back wall** sits above the point boxes. It is part of the generated layout (`craps/layout_<theme>`,
  visual §4.1), and every bet area uses the layout rects of visual §4.1.
- **Chips on the layout:**
  - Your flat bet sits as a stack at the right third of its area, and your odds stack sits in the
    **odds lane** directly below it, "behind the line" (visual §4.1).
...
Panel changes (visual §5, §6):
- The panel becomes a full-screen **arena** (`dice_duel_arena_<theme>`, 360 × 140) with two lanes, a centre
  line and a "vs" badge (pixel art, with no letters: two crossed dice).
- Each lane has a dice cup (40 × 40, 6 frames) at its outer edge; the dice are 40 px (16-px art ×2).
...
> **Superseded** by `docs/design/visual/tables.md` §8 (paths `gui/tables/…`, `gui/sprites/tables/…`,
> `entity/roulette/wheel_<theme>.png`, `entity/craps/dice.png`, Java only). Kept for history.
...
- The generator is the shared **`tools/assets/gen-assets.mjs`** (global task S1, module `tables.mjs`). It writes
  the Java textures from one palette.
...
| # | Path (`textures/`) | Size | Frames | Content |
|---|------------------------------------------|------|--------|---------|
...
### 4.2 Glyphs (`font/default.json` bitmap provider over `glyph_e1.png`)
...
There are **12 new sound events** (§0.8), plus subtitle keys,.
...
| J-A1 | **Assets:** a `tables` section in `scripts/gen-fx-assets.py` (T1–T22, T23 cell, glyphs E1C0–E1CB) that writes `sounds.json` entries (§0.8); lang keys (§6) | `scripts/gen-fx-assets.py`, `src/main/sounds/*/sounds.json`, STRINGS.md + `gen_lang.py` | global S1 (generator skeleton), J3 (Java glyph font) |
