...
The pipeline is five reusable workflows (`on: workflow_call`). They contain nothing project-specific: every project
detail comes from the calling workflow's inputs and from repository variables.

> **Shared with Enchantaholic.** The same five `reusable-*.yml` workflows, `scripts/curseforge-upload.sh`,
> `scripts/check-inlined-script.sh`, `scripts/test/` and `.github/templates/release-caller.yml` exist, byte-identical,
> in [`nezo32/enchantaholic`](https://github.com/nezo32/enchantaholic), which is their canonical copy for now (the
> template and the examples below point there). Burmaldaholic calls its own copies by local path
> (`uses: ./.github/workflows/reusable-*.yml`); only `ci.yml` and `release.yml` are project-specific. **Do not edit
> the shared files in only one repository.** The recommended next step is to move them into one shared repository
> that both projects call; see [section 11](#11-moving-to-a-dedicated-repository). Until then, check that the copies
> match after every change:
> ```bash
> for f in .github/workflows/reusable-*.yml .github/templates/release-caller.yml scripts/*.sh scripts/test/*; do
>   [ -f "$f" ] && { cmp -s "$f" ../enchantaholic/"$f" || echo "differs: $f"; }
> done
> ```
...
```
See Burmaldaholic's [`ci.yml`](../../.github/workflows/ci.yml): it runs the Gradle build in a matrix
(`-Pmc=26.2` / `-Pmc=26.3`), skips editions a PR does not touch with path filters, and aggregates all jobs into one
`ci-ok` job, the only required status check (a required check skipped by a path filter would block the PR).
...
The workflows are duplicated in `nezo32/enchantaholic` and `nezo32/burmaldaholic` for now (identical files, each repo
calling its own copy by local path). The recommended next step is one dedicated `nezo32/gh-workflows` repository that
both projects call, so a fix lands once instead of twice:
...
3. In Enchantaholic **and** Burmaldaholic, switch the local `./.github/workflows/reusable-*.yml` references in
   `ci.yml` and `release.yml` to the remote ones, delete the local `reusable-*.yml`, `scripts/` and
   `.github/templates/` copies, and drop the `scripts` job and the template lint from their `ci.yml`.
4. Point the template's `uses:` lines and its guide link at the new repository.
