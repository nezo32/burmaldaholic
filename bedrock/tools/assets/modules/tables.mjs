...
export default function generate() {
  const out = [];
  const gui = (rel, img) => out.push(png('java', `${GUI}/${rel}.png`, img));
  ...
  const anim = (rel, img, frametime, fw, fh) => {
    out.push(png('java', `${SPR}/${rel}.png`, img));
    out.push(json('java', `${SPR}/${rel}.png.mcmeta`, { animation: { frametime, width: fw, height: fh } }));
  };
  ...
  for (const t of THEME_ORDER) {
    ...
    out.push(png('java', `${T}/entity/roulette/wheel_${t}.png`, W.worldTexture(t)));
  }
  ...
  out.push(png('java', `${SPR}/roulette/cell_win.png`, Ro.cellWin()));
  ...
  out.push(png('java', `${T}/entity/craps/dice.png`, diceWorld()));
  ...
  for (const [owner, set] of Object.entries(PARTICLES))
    for (const [id, n] of Object.entries(set)) for (let f = 0; f < n; f++) out.push(png('java', `${T}/particle/${owner}/${id}_${f}.png`, particle(id, f)));
