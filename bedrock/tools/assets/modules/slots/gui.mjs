// Java GUI sprites (animation/slots.md §3.3, §9.1), backdrops, Showdown sprites and Bedrock form icons (§9.2).
...
// ---- Bedrock form icons (32 × 32 = 16-px art ×2) ------------------------------------------------------------------------
function formIcon(name) {
  const img16 = image(16, 16);
  const G = (rows, pal) => rows.forEach((row, j) => [...row].forEach((ch, i) => ch !== '.' && put(img16, i, j, c(pal[ch]))));
  const chip = { k: INK, r: '#D83440', l: '#FF6E6A', w: '#F4ECF8' };
  switch (name) {
    case 'spin':
    case 'auto':
      ring(img16, 8, 8, 6.5, 2, c('#FFD640'));
      for (let y = 1; y <= 7; y++) for (let x = 8; x <= 15; x++) if (x - 8 < 7 - y) put(img16, x, y, [0, 0, 0, 0]);
      G(['..........', '..........', '.........k', '........kyk', '.......kyyyk', '........kyk', '.........k'].map((s) => s.padEnd(16, '.')), { k: '#5C3A00', y: '#FFD640' });
      if (name === 'auto') G(['................', '................', '................', '................', '................', '.....kk.........', '.....kwk........', '.....kwwk.......', '.....kwk........', '.....kk.........'], { k: INK, w: '#FFFFFF' });
      break;
    case 'stop':
      fill(img16, 3, 3, 10, 10, c(INK));
      fill(img16, 4, 4, 8, 8, c('#E83030'));
      fill(img16, 4, 4, 8, 1, c('#FF8A80'));
      break;
    case 'bet_down':
    case 'bet_up':
      disc(img16, 8, 8, 7.5, c(INK));
      disc(img16, 8, 8, 6.5, c(chip.r));
      ring(img16, 8, 8, 5, 1, c(chip.w));
      fill(img16, 5, 7, 6, 2, c('#FFFFFF'));
      if (name === 'bet_up') fill(img16, 7, 5, 2, 6, c('#FFFFFF'));
      break;
    case 'buy':
      G([
        '................',
        ...
        '....kbbbbbbk....',
        '.....kkkkkk.....',
      ], { k: '#2E1A08', b: '#A8692A', y: '#FFD640', Y: '#FFF4B0' });
      break;
    case 'paytable':
      G([
        '................',
    ...
    case 'leave':
      G([
        ...
        '..kbbbbbk.......',
        '..kkkkkkk.......',
      ], { k: '#2E1A08', b: '#8A5220', y: '#FFD640' });
      break;
    case 'chest':
      over(img16, art(ICON.chest_closed));
      break;
    default:
      throw new Error(`form icon ${name}`);
  }
  return shadow(outline(scaleUp(img16, 2), c(INK), { k: 0.7 }), c(INK), 1, 1, 0.35);
}
...
export function guiOutputs() {
  ...
  const sprite = (rel, img, meta) => {
    out.push(png('java', `${SPR}/${rel}.png`, img));
  ...
  const pvp = (rel, img, meta) => {
    out.push(png('java', `${PVP}/${rel}.png`, img));
  ...
  for (const n of ['spin', 'stop', 'bet_down', 'bet_up', 'buy', 'auto', 'paytable', 'leave', 'chest']) out.push(png('bedrock', `packs/slots/RP/textures/ui/burmaldaholic/slots/${n}.png`, formIcon(n)));
  return out;
}
