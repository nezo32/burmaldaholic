...
/** Win-plane glyph (E3): the symbol brightened 20 % with a 1 px gold rim inside the cell. */
export function winGlyph(img) {
  const out = brighten(img, 1.2);
  const n = img.w;
  for (let k = 1; k < n - 1; k++) {
    for (const [x, y] of [[k, 0], [k, n - 1], [0, k], [n - 1, k]]) put(out, x, y, GOLD);
  }
  // rounded corners stay empty; inner corner accents
  for (const [x, y] of [[1, 1], [n - 2, 1], [1, n - 2], [n - 2, n - 2]]) put(out, x, y, GOLD);
  return out;
}
